...
Future<String> _accessToken() async {
  if (
      ...
      (_accessTokenStore != null ? _accessTokenStore.isEmpty : true)
          // if we do have one check it hasn't expired.
          ||
          (_accessTokenExpires != null
              ? DateTime.now().isAfter(_accessTokenExpires)
              : true)) {
    try {
      var response = await http.post('${_accountsApiUrl}token', headers: {
      ...
      if (response.statusCode == 200) {
        ...
        _accessTokenStore = decoded['access_token'];
...
Future<bool> _searchForAlbumId(SpotifyAlbum album) async {
  ...
  try {
    var response = await http.get(
        ...
        Uri.parse(_apiUrl + 'search').replace(
            queryParameters: {'q': album.searchTerm, 'type': 'album', 'market': _market}),
        headers: {
      ...
      if (decoded['albums']['items'].length > 0) {
        ...
        if (appleResponse.statusCode == 200) {
          ...
          if (decoded['resultCount'] > 0) {
            ...
            album.releaseDate = DateFormat('yyyy-MM-dd')
                .parse(decodedAlbum['releaseDate'].toString().substring(0, 10));
          }
        }
      }
    } else {
  ...
  try {
    if (album._appleTrackCount != null) {
      var itunesResponse = await http.get(
          ...
          Uri.parse(_itunesUrl + 'search').replace(queryParameters: {
        'term': album.title,
        'limit': album._appleTrackCount.toString(),
        'country': _market,
        'entity': 'musicTrack',
        'media': 'music'
      }));
      ...
      if (itunesResponse.statusCode == 200) {
        ...
        for (Map<String, dynamic> track in decoded['results']) {
          ...
          album.tracks[trackNum] = new SpotifyTrack(
        ...
        print(
            'something went wrong, status code: ${itunesResponse.statusCode.toString()}');
      }
    } else {
      var response = await http.get(
          // unlike with a post, the get function doesn't allow us to send parameters, to get around this we have to build a uri encoded query string.
          Uri.encodeFull(_apiUrl + 'albums/${album.id}' + '?market=$_market'),
          headers: {
            // the access token we get from spotify is already encoded so no extra work needs to be done.
            'Authorization': 'Bearer ${await _accessToken()}'
          });
